...
while temp ~= -1
  %  gen=1;         %代数
     ...
     temp=fgetl(fid);
     ntype=str2num(temp);  % 每种原子的个数
     if isempty(ntype)
         tmp=fgetl(fid);
         ntype = str2num(tmp);
     end
     natom=sum(ntype);     % 原子的总数
     temp=fgetl(fid);         %  the direct
     Sel_POS = findstr(temp, 'S'); % 有没有 Sel 字段
     
     if Sel_POS    %　如果有　＇wel ' , 再读一行 。
         temp = fgetl ( fid );
     end
     for ll = 1 : natom
         temp=fgetl(fid);
         D_position = findstr( temp ,'.'); % 小数点所在位置
         for kp = 1 : length( D_position ) % 小数点个数
             D_p = D_position( kp );
  ...
  %   temp=fgetl(fid);  % 跳到文件开头
     fp=fopen(filename_new,'w');
     fprintf(fp,['TITLE   ' filename_new ' \n']);
     fprintf(fp,'CELL\n');
     lat_abc=latConverter(Bulk_lat);       % 晶胞参数
