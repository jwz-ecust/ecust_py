...
import sys
import os
from vasptools.utils import readCHGCAR
...
def chgsplit():
    ...
    rho_down = list()
    for i in range(Npts):
        rho_up.append(   ( rho_up_p_down[i] + rho_up_m_down[i] ) / 2. )
        rho_down.append( ( rho_up_p_down[i] - rho_up_m_down[i] ) / 2. )
    ...
    fchgcar_minus = open( "CHGCAR_minus", 'w')
    ...
    Npts = len(rho_up)
    while i < Npts:
        line_up = line_down = line_minus = ""
        for j in range(5):
            line_up   += "%18.11e " % rho_up[i]
            line_down += "%18.11e " % rho_down[i]
            line_minus += "%18.11e" % (rho_up[i]-rho_down[i])
            i += 1
            if i >= Npts:
                break
        line_up   += "\n"
        ...
        fchgcar_minus.write(line_minus)
